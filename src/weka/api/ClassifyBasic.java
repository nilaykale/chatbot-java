...
public class ClassifyBasic {

	static String filePath = "input.json";

	public static void main(String[] args) throws Exception {
		System.out.println("ClassifyBasic");
		try {
		...
		FilteredClassifier nesne = new FilteredClassifier();
		StringToWordVector filter = new StringToWordVector();
		nesne.setFilter(filter);
		nesne.setClassifier(new NaiveBayes());
		nesne.buildClassifier(train);

		Instances labeled = new Instances(test);

		// label instances
		for (int i = 0; i < test.numInstances(); i++) {
			double clsLabel = nesne.classifyInstance(test.instance(i));
